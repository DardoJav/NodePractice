...
export const handlePolicies = policies => (req, res, next) => {
    ...
    if (policies.includes('PREMIUM') && policies.includes('ADMIN')) {
...
export const sendPurchaseEmail = async (res, ticket) => { 
    try{
        let config = {
            service: 'gmail',
            auth: {
...
export const sendEmail = async (reason, product, user) => { 
    try{
        let subject = ''
        let html = ''
        switch (reason) {
            case 'userDelete':
                subject = '[Coder Ecommerce API] User account deleted'
                html = `<h1>[Coder Ecommerce API] User account deleted </h1><hr />Your account with email: ${user.email} has been deleted due to inactivity`
                break;
            case 'productDelete':
                subject = '[Coder Ecommerce API] Product deleted'
                html = `<h1>[Coder Ecommerce API] Product with id: ${product._id} deleted </h1><hr />Your product: <b>${product.title}</b> has been deleted`
                break;
            case 'productUpdate':
                subject = '[Coder Ecommerce API] Product updated'
                html = `<h1>[Coder Ecommerce API] Product with id: ${product._id} updated </h1><hr />Your product: <b>${product.title}</b>  has been updated`
                break;
            default:
                break;
        }
        const email = !product ? user.email: product.owner
        ...
        logger.error(`error on sendEmail: ${err.message}`)
            return {
                statusCode: 500,
